...
- Each connection is driven by a `ConnTask` (`conn_task.rs`) — a single-owner tokio task that holds all mutable protocol state (send/receive buffers, congestion control and pacing, loss lists, FEC, crypto, TSBPD) as plain fields, no `Mutex` anywhere, fed `NetEvent`s over an mpsc channel. `recv_loop::run` is a separate, stateless task that only parses inbound UDP and routes it — and it is spawned **per multiplexer (per UDP port), not per connection**: a caller creates its own multiplexer and spawns both, whereas a listener spawns one `recv_loop` for the whole port and each accepted connection then spawns only its `ConnTask`
